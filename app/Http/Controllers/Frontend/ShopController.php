...
class ShopController extends Controller
{
    ...
    public function ShopFilter(Request $request){
        $data = $request->all();  // Change $date to $data
        ...
        if (!empty($data['category'])) {
            foreach ($data['category'] as $category) {
                if (empty($catUrl)) {
                    $catUrl .= 'category=' . $category;  // Remove $ before category
                } else {
                    $catUrl .= ',' . $category;
                }
            }
        ...
        if (!empty($data['brand'])) {
            foreach ($data['brand'] as $brand) {
                if (empty($brandUrl)) {
                    $brandUrl .= '&brand=' . $brand;  // Remove $ before category
                } else {
                    $brandUrl .= ',' . $brand;
                }
            }
        ...
        if(!empty($data['price_range'])){
            ...
            $priceRangUrl .= '&prie='.$data['price_range'];
        }
