@extends('frontend.master_dashobard')
@section('main')
                ...
                <form action="{{ route('shop.filter') }}" method="POST">
                    ...
                    <div class="price-filter">
                        <div class="price-filter-inner">
                            <div id="slider-range" class="price-filter-range" min="0" max="2000"></div>
                            <input type="hidden" id="price-range" name="price-range" value="">
                            <input type="text" id="amount" value="$0 - $2000" readonly="">
                            <br><br>
                            <button type="submit" id="filter-btn" class="btn btn-sm btn-default"><i class="fi-rs-filter mr-5"></i> Filter</button>
                        </div>
                    </div>
                    ...
                    @endphp
                     @endif
                        ...
                        <div class="custome-checkbox">
                            ...
                            <label class="form-check-label" for="exampleBrand{{$brand->id}}"><span>{{ $brand->brand_name }}({{ count($products) }})</span></label>
                            <br />
                        </div>
                        @endforeach
                    </div>
...
<script>
    $(document).ready(function(){
        if($('#slider-range').length > 0){
            const max_price = parseInt($('#slider-range').attr('max')); 
            const min_price = parseInt($('#slider-range').attr('min')); 
